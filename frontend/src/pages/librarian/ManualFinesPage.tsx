...
const ManualFinesPage: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const message =
        error.response?.data?.message || "Jarima yaratishda xatolik yuz berdi.";
      toast.error(message);
    } finally {
      setLoading(false);
    }
        ...
        <div className="md:col-span-8">
          <Paper sx={{ p: 3 }}>
            <Typography variant="h6" sx={{ mb: 2 }}>
              2. Jarima ma'lumotlarini kiriting{" "}
              {selectedUser && `uchun: ${selectedUser.firstName}`}
            </Typography>
            <Box
              sx={{
                display: "flex",
            ...
            >
              <Autocomplete
                ...
                getOptionLabel={(option) =>
                  `${option.title} - ${option.author || "Muallif noma'lum"}`
                }
                ...
                noOptionsText={
                  bookSearch.length < 2
                    ? "Kitob qidirish uchun kamida 2 ta harf kiriting"
                    : "Kitob topilmadi"
                }
                renderInput={(params) => (
                  <TextField
              ...
              <Autocomplete
                ...
                options={
                  selectedBook && selectedBook.copies
                    ? selectedBook.copies.map((c) => c.barcode)
                    : []
                }
                ...
                renderInput={(params) => (
                  <TextField
                    ...
                    helperText={
                      selectedBook && selectedBook.copies
                        ? `Jami ${selectedBook.copies.length} ta nusxa`
                        : null
                    }
                  />
                )}
